...
(function () {
  ...
  window.KC_IMAGES = {
    ...
    highlightRiverCrossing: "gallery-route.jpg",
    ...
    // "About The Wilderness Expedition" — one image slot per highlight
    // card. Leave blank ("") until you upload a photo.
    hlNotConventional: "Rock Overhang Rest Stop.jpg",
    hlWilderness: "highlight-river-crossing.jpg",
    hlRarelyVisited: "gallery-river.jpg"
  };
  ...
  window.KC_CONTENT = {
      ...
      highlights: [
        {
          ...
          imagesEnabled: true,
          images: [window.KC_IMAGES.hlNotConventional]
        },
        {
          icon: "water",
        ...
        },
        {
          ...
          imagesEnabled: true,
          images: [window.KC_IMAGES.hlWilderness]
        },
        {
          icon: "eco",
          ...
          imagesEnabled: true,
          images: [window.KC_IMAGES.hlRarelyVisited]
        }
      ]
    },
    ...
    hero: {
      ...
      // page number (2-7) from Telegram admin to override it.
    },

    // Persistent header/mobile-menu CTA button (the one that stays
    // visible while scrolling, separate from the hero's own "Explore"
    // button above). Label already editable from Telegram admin bot →
    // ✏️ Edit Website Text → UI Labels → bookNow; the page it jumps to
    // is set here.
    headerCta: {
      targetPage: 2
    },
