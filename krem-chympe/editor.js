...
(function(){
  ...
  function sanitizeOverrides(o){
    ...
    //
    // Text/image entries additionally require `original` — a snapshot of the element's
    // content at the moment the edit was made. tag+className alone isn't unique enough:
    // several unrelated buttons across the app share the exact same class string (e.g.
    // every "kc-whatsapp-btn"), so a saved edit could otherwise get replayed onto a
    // completely different element that happens to land on the same position and share
    // that class, silently wiping its real content. Entries saved before this fix have
    // no `original`, so they're dropped here rather than risk that collision.
    ['texts','images'].forEach(function(kind){
      Object.keys(o[kind]).forEach(function(path){
        var v = o[kind][path];
        if(!v || typeof v !== 'object' || typeof v.fp !== 'string' || typeof v.original !== 'string'){
          delete o[kind][path];
        }
      });
    });
  ...
  function applyOverrides(){
    ...
    Object.keys(overrides.texts).forEach(function(path){
      ...
      // Belt-and-suspenders on top of the fingerprint: only apply if this element's
      // current text still matches what was there when the edit was saved (fresh
      // render), or already matches the saved value (already applied). Anything else
      // means this path+class now belongs to a different element entirely — leave it.
      var currentText = el.textContent;
      if(currentText !== entry.original && currentText !== entry.value) return;
    ...
    Object.keys(overrides.images).forEach(function(path){
      ...
      if(currentSrc !== entry.original && el.src !== entry.value) return;
      el.src = entry.value;
    });
  ...
  function setupFileInput(){
    ...
    fileInput.addEventListener('change', function(){
      ...
      compressImageFile(file, function(dataUrl){
        if(!dataUrl) return;
        var origSrc = pendingImgEl.dataset.kcOrig || pendingImgEl.getAttribute('src');
        pendingImgEl.src = dataUrl;
        var p = pathOf(pendingImgEl);
        if(p){ overrides.images[p] = {value:dataUrl, original:origSrc || '', fp:fingerprint(pendingImgEl)}; saveOverrides(); }
      });
      fileInput.value = '';
    });
  }
  ...
  function refreshHandlers(){
    ...
    if(mode==='text'){
      ...
      for(var k=0;k<all.length;k++){
        if(isLeafTextEl(all[k])){
          // Snapshot the pre-edit text now, before the user touches it, so the saved
          // entry can record what was really there — not just this element's tag+class.
          all[k].dataset.kcOrig = all[k].textContent;
    ...
    if(mode==='image'){
      ...
      for(var m=0;m<allImgs.length;m++){
        allImgs[m].dataset.kcOrig = allImgs[m].getAttribute('src');
  ...
  document.addEventListener('focusout', function(e){
    if(mode==='text' && e.target && e.target.classList && e.target.classList.contains('kc-editable-text')){
      ...
      if(p){ overrides.texts[p] = {value:e.target.textContent, original:e.target.dataset.kcOrig || '', fp:fingerprint(e.target)}; saveOverrides(); }
    }
  }, true);
  ...
  function onPointerDown(e){
    ...
    pressTimer = setTimeout(function(){
      ...
      if(confirm('Remove this image?')){
        var origSrc = img.dataset.kcOrig || img.getAttribute('src');
        img.src = TRANSPARENT;
        var p = pathOf(img);
        if(p){ overrides.images[p] = {value:TRANSPARENT, original:origSrc || '', fp:fingerprint(img)}; saveOverrides(); }
      }
    }, LONG_PRESS_MS);
  }
